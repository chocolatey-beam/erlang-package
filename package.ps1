...
New-Variable -Name latest_erlang_tag -Option Constant `
  -Value ($erlang_tags | Where-Object { $_.name -match '^OTP-27\.[0-9]\.[0-9]\.[0-9]$' } | Sort-Object -Descending { $_.name } | Select-Object -First 1)
...
if ($PackAndTest)
{
    ...
    & choco install erlang $arg_debug $arg_verbose --yes --version=$otp_version --skip-virus-check --source ".;https://chocolatey.org/api/v2/"
    if ($LASTEXITCODE -eq 0)
    {
